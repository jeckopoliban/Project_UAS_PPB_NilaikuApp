...
@section('title', 'Buat Template Grading')
@section('header', 'Buat Template Grading Baru')

@section('content')
<div class="max-w-4xl">
    <form method="POST" action="{{ route('admin.grading-template.store') }}" class="space-y-6">
        @csrf
        ...
        <div class="bg-white rounded-app-card border border-border-subtle p-6 shadow-app-card">
            <div class="grid gap-4">
                <div>
                    ...
                    <input type="text" name="nama_template" value="{{ old('nama_template') }}" class="w-full px-4 py-3 bg-slate-50 border border-border-subtle rounded-app-input text-text-heading placeholder:text-text-muted focus:outline-none focus:border-primary-start" placeholder="Contoh: Template 2024" required>
                    @error('nama_template')
                ...
                <div class="flex items-center gap-3">
                    <input type="checkbox" id="is_default" name="is_default" value="1" {{ old('is_default') ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300 text-brand-blue focus:ring-brand-blue/50">
        ...
        <div class="bg-white rounded-app-card border border-border-subtle p-6 shadow-app-card">
            <div class="flex items-center justify-between gap-4 mb-4">
                <div>
                    ...
                    <p class="text-sm text-text-body">Tambahkan rentang nilai, huruf mutu, dan indeks untuk template ini.</p>
                </div>
                ...
                <div id="items" class="space-y-3">
                    ...
                    @else
                        <div class="flex gap-2 items-center" data-index="0">
                            <input type="number" name="items[0][batas_bawah]" placeholder="80" class="w-24 rounded-app-input border border-border-subtle bg-slate-50 px-3 py-2" step="0.01" required>
                            <input type="number" name="items[0][batas_atas]" placeholder="100" class="w-24 rounded-app-input border border-border-subtle bg-slate-50 px-3 py-2" step="0.01" required>
                            <input type="text" name="items[0][huruf_mutu]" placeholder="A" class="flex-1 rounded-app-input border border-border-subtle bg-slate-50 px-3 py-2" required>
                            <input type="number" name="items[0][indeks]" placeholder="4" class="w-20 rounded-app-input border border-border-subtle bg-slate-50 px-3 py-2" step="0.01" min="0" max="4" required>
                            <button type="button" class="text-rose-600 remove-item">Hapus</button>
                        </div>
                    @endif
                </div>
        ...
        <div class="flex flex-col sm:flex-row gap-3">
            <button type="submit" class="flex-1 inline-flex items-center justify-center rounded-app-pill bg-gradient-button px-6 py-3 text-sm font-semibold text-white shadow-app-soft transition hover:opacity-95">Simpan</button>
