...
@section('header', 'Edit Tahun Akademik')

@section('content')
<div class="rounded-app-card border border-border-subtle bg-white p-6 shadow-app-soft max-w-md">
    <form method="POST" action="{{ route('portal.tahun-akademik.update', $item->id) }}" class="space-y-6">
        @csrf
        @method('PATCH')

        <div>
            <label class="block text-sm font-semibold text-slate-700 mb-2">Nama Tahun Akademik</label>
            <input type="text" name="nama" value="{{ old('nama', $item->nama) }}" class="w-full rounded-app-pill border border-border-subtle bg-slate-50 px-4 py-3 text-slate-900 placeholder-slate-400 focus:border-brand-blue focus:ring-brand-blue/50 focus:outline-none" required>
            @error('nama')
        ...
        <div class="flex items-center gap-3">
            <input type="checkbox" id="status_aktif" name="status_aktif" value="1" {{ old('status_aktif', $item->status_aktif) ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300 text-brand-blue focus:ring-brand-blue/50">
