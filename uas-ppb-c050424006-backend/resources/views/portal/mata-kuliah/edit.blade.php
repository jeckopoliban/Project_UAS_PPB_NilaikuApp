...
<div class="rounded-app-card border border-border-subtle bg-white p-6 shadow-app-soft max-w-md">
    <form method="POST" action="{{ route('portal.mata-kuliah.update', $item->id) }}" class="space-y-6">
        @csrf
        @method('PATCH')
        ...
        <div>
            ...
            <select name="tahun_akademik_id" class="w-full rounded-app-pill border border-border-subtle bg-slate-50 px-4 py-3 text-slate-900 focus:border-brand-blue focus:ring-brand-blue/50 focus:outline-none" required>
                ...
                @foreach ($tahunAkademiks as $tahun)
                    <option value="{{ $tahun->id }}" {{ old('tahun_akademik_id', $item->tahun_akademik_id) == $tahun->id ? 'selected' : '' }}>{{ $tahun->nama }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input type="text" name="nama_mk" value="{{ old('nama_mk', $item->nama_mk) }}" class="w-full rounded-app-pill border border-border-subtle bg-slate-50 px-4 py-3 text-slate-900 placeholder-slate-400 focus:border-brand-blue focus:ring-brand-blue/50 focus:outline-none" required>
            @error('nama_mk')
                <p class="text-rose-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="number" name="sks" value="{{ old('sks', $item->sks) }}" min="1" max="6" class="w-full rounded-app-pill border border-border-subtle bg-slate-50 px-4 py-3 text-slate-900 placeholder-slate-400 focus:border-brand-blue focus:ring-brand-blue/50 focus:outline-none" required>
            @error('sks')
                <p class="text-rose-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
